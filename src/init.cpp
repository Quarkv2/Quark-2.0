...
std::string HelpMessage(HelpMessageMode mode)
{
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "quark2d.pid") + "\n";
#endif
    ...
    if (GetBoolArg("-help-debug", false))
    {
        ...
        strUsage += "  -sporkkey=<privkey>    " +  _("Enable spork administration functionality with the appropriate private key.") + "\n";        
    }
    ...
    strUsage += "\n" + _("RPC SSL options: (see the Quark2 Wiki for SSL setup instructions)") + "\n";
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin and Quark 2.0 Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Quark 2.0 Core is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Quark 2.0 Core is probably already running."), strDataDir));
#ifndef WIN32
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Quark 2.0 Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Quark 2.0 Core to complete") << "\n";
